...
class Triangle:
    ...
    def isValidLength(self, sideA, sideB, sideC):
        if sideA <= 0 or sideB <= 0 or sideC <= 0:
            return False
        return True

    def isValidTriangleSides(self, sideA, sideB, sideC):
        if sideA + sideB <= sideC or sideB+sideC <= sideA or sideC+sideA <= sideB:
            return False
        return True

    def isEquilateralTriangle(self, sideA, sideB, sideC):
        if sideA == sideB and sideB == sideC:
            return True
        return False

    def isIsoscelesTriangle(self, sideA, sideB, sideC):
        if sideA == sideB or sideA == sideC or sideB == sideC:
            return True
        return False

    def isRightTriangle(self, sideA, sideB, sideC):
    ...
    def testTriangle(self, sideA, sideB, sideC):
        ...
        if self.isRightTriangle(sideA, sideB, sideC):
            response = response + " right"

        if (not self.isRightTriangle(sideA, sideB, sideC) and 
            not self.isIsoscelesTriangle(sideA, sideB, sideC)):
